...
def read_article(newspaper)
  number_of_articles = newspaper[:response][:results].size
  newspaper[:response][:results][rand(number_of_articles)][:views] += 1
end

def display_views(newspaper)
  newspaper[:response][:results].each do |article|
    puts "#{article[:webTitle]}, views: #{article[:views]}"
  end
end
